...
dir <- make.dir('./02_NMDS_tSNE/02_output')
...
sink(sprintf('%s/tSNE_Relative_Family_EnvFit_log.txt', dir$tabledir))
...
geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
       data = en_coord_cont, size =0.7, alpha = 0.5, colour = "red")+
...
ggsave(plot=g1, filename=sprintf('%s/NMDS_Relative_EnvFit_Family.pdf', dir$figdir), h=5, w=6)


#### PERMANOVA

adonis2(comm2/rowSums(comm2) ~ env3$pH + env3$DO + env3$Eel.Activity, permutations=10000, method="bray", thread=4)

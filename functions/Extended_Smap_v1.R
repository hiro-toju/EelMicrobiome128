      ...
      if (regularized)
      {
        ...
        if (E == 1)
        {
          ...
          if(is.null(lambda)){
            ...
            if(!is.matrix(coef(fit, s = fit$lambda))) {
            		coeftmp=matrix(coef(fit, s = fit$lambda))
            	}else{
            		coeftmp=coef(fit, s = fit$lambda)
            }
            smap_coefficient_val <- matrix(t(coeftmp), nrow = 1)[c(2:(NCOL(vectors) + 1), 1)]
          }
        }
      } else {

...
start <- proc.time()[3]
for (j in 1:n.ex.rep) {
w <- subset(pH.int, pH.int$Parameter==sprintf('T%s_pH', j))
v <- w[1:(nrow(w)-lag), ]

data.p <- foreach(i=1:ncol(dcomm3[[j]])) %pHpar% cbind(v, na_interpolation(as.numeric(dcomm3[[j]][-c(1:lag), i]), option = "linear"))

pH.r <- c()
pH.r <- foreach(i=1:ncol(dcomm3[[j]]), .combine='rbind', .errorhandling="pass") %pHpar% synctest(as.matrix(data.p[[i]][, c(3,4)]), nboot=n.boot)

corr <- foreach(i=1:ncol(dcomm3[[j]]), .combine='rbind', .errorhandling="pass") %pHpar% synctest(as.matrix(data.p[[i]][, c(3,4)]), nboot=1)
...
start <- proc.time()[3]
for (j in 1:n.ex.rep) {
w <- subset(DO.int, DO.int$Parameter==sprintf('T%s_DO', j))
v <- w[1:(nrow(w)-lag), ]

data.p <- foreach(i=1:ncol(dcomm3[[j]])) %dopar% cbind(v, na_interpolation(as.numeric(dcomm3[[j]][-c(1:lag), i]), option = "linear"))
...
start <- proc.time()[3]
for (j in 1:n.ex.rep) {
w <- subset(AS.int, AS.int$Parameter==sprintf('T%s_AS', j))
v <- w[1:(nrow(w)-lag), ]
...
w <- cbind(v.AS, v.pH$pH.value)
AS.pH <- w[1:(nrow(w)-lag), ]

data.p <- foreach(i=1:ncol(dcomm3[[j]])) %dopar% cbind(AS.pH, na_interpolation(as.numeric(dcomm3[[j]][-c(1:lag), i]), option = "linear"))
...
w <- cbind(v.AS, v.DO$DO.value)
AS.DO <- w[1:(nrow(w)-lag), ]

data.p <- foreach(i=1:ncol(dcomm3[[j]])) %dopar% cbind(AS.DO, na_interpolation(as.numeric(dcomm3[[j]][-c(1:lag), i]), option = "linear"))
...
for(i in 1:n.ex.rep) {
	...
	write.table(result.all[[i]], file=sprintf("%s/Surrogate_ASVs_CommDealy%s_All_th%s_Tank%s_%s.txt", dir$tabledir, lag, sample.th, i, n.boot), sep="\t", quote=FALSE, row.names=FALSE)
}

stopCluster(cluster)

saveRDS(result.all, sprintf("%s/Surrogate_ASVs_CommDealy%s_All_th%s_Tank%s_%s.rds", dir$rdsdir, lag, sample.th, i, n.boot))

############################################################################

save.image(sprintf("%s/Surrogate_ASVs_CommDealy%s_th%s_%s.RData", dir$rdatadir, lag, sample.th, n.boot))

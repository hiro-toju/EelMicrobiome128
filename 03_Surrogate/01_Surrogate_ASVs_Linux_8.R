...
#### 2022.03.23 Hirokazu Toju 
#### R 4.1.2
#### 
############################################################################

#setwd("/Users/toju/Dropbox/YAJIMA_Daii/Statistics_R")
setwd("/media/toju/HDD12TBA/Toju/Statistics_R_v2")
...
sample.th <- 30
n.boot <- 100000 # (100000)

source('functions/functions.R')
source('functions/rEDM075_surrogate_codes.R')

# -- Create directory to save
dir <- make.dir('03_Surrogate/01_output')
...
cluster = parallel::makeCluster(parallel::detectCores(logical = FALSE), type = "FORK")
...
start <- proc.time()[3]
for (j in 1:n.ex.rep) {
v <- subset(DO.int, DO.int$Parameter==sprintf('T%s_DO', j))
data.p <- foreach(i=1:ncol(dcomm3[[j]])) %dopar% cbind(v, na_interpolation(as.numeric(dcomm3[[j]][, i]), option = "linear"))
DO.r <- c()
...
start <- proc.time()[3]
for (j in 1:n.ex.rep) {
v <- subset(AS.int, AS.int$Parameter==sprintf('T%s_AS', j))
data.p <- foreach(i=1:ncol(dcomm3[[j]])) %dopar% cbind(v, na_interpolation(as.numeric(dcomm3[[j]][, i]), option = "linear"))
AS.r <- c()
...
AS.pH <- cbind(v.AS, v.pH$pH.value)

data.p <- foreach(i=1:ncol(dcomm3[[j]])) %dopar% cbind(AS.pH, na_interpolation(as.numeric(dcomm3[[j]][, i]), option = "linear"))
...
AS.DO <- cbind(v.AS, v.DO$DO.value)

data.p <- foreach(i=1:ncol(dcomm3[[j]])) %dopar% cbind(AS.DO, na_interpolation(as.numeric(dcomm3[[j]][, i]), option = "linear"))
...
P_Partial.r_AS.contr.pH <- list()
P_Partial.r_AS.contr.DO <- list()
P_AS <- list()
P_pH <- list()
P_DO <- list()

for (i in 1:n.ex.rep) {
  P_Partial.r_AS.contr.pH[[i]] <- apply(matrix(as.numeric(unlist(AS.result_Contr.pH[[i]][,3:4])), ncol=2), 1, FUN=min)
  P_Partial.r_AS.contr.DO[[i]] <- apply(matrix(as.numeric(unlist(AS.result_Contr.DO[[i]][,3:4])), ncol=2), 1, FUN=min)
  P_AS[[i]] <- apply(matrix(as.numeric(unlist(AS.result[[i]][,3:4])), ncol=2), 1, FUN=min)
  P_pH[[i]] <- apply(matrix(as.numeric(unlist(pH.result[[i]][,3:4])), ncol=2), 1, FUN=min)
  P_DO[[i]] <- apply(matrix(as.numeric(unlist(DO.result[[i]][,3:4])), ncol=2), 1, FUN=min)
}

result.all <- list()

for(i in 1:n.ex.rep) {
	result.all[[i]] <- cbind(as.matrix(AS.result_Contr.pH[[i]][, 1:2]), P_Partial.r_AS.contr.pH[[i]], as.matrix(AS.result_Contr.DO[[i]][, 2]), P_Partial.r_AS.contr.DO[[i]], as.matrix(AS.result[[i]][,2]), P_AS[[i]], as.matrix(pH.result[[i]][,2]), P_pH[[i]], as.matrix(DO.result[[i]][,2]), P_DO[[i]])
	#class(result.all[[i]]) <- 'data.frame'
	colnames(result.all[[i]]) <- c('ASV.ID', 'AS.result_Contr.pH', 'P_Partial.r_AS.contr.pH', 'AS.result_Contr.DO', 'P_Partial.r_AS.contr.DO', 'AS.result', 'P_AS', 'pH.result', 'P_pH', 'DO.result', 'P_DO')
	
	write.table(result.all[[i]], file=sprintf("%s/Surrogate_ASVs_All_th%s_Tank%s_%s.txt", dir$tabledir, sample.th, i, n.boot), sep="\t", quote=FALSE, row.names=FALSE)
}


stopCluster(cluster)

saveRDS(result.all, sprintf("%s/Surrogate_ASVs_th%s_%s.rds", dir$rdsdir, sample.th, n.boot))

############################################################################

save.image(sprintf("%s/Surrogate_ASVs_th%s_%s.RData", dir$rdatadir, sample.th, n.boot))
